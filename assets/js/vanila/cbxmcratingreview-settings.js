(function ($) {
    ...
    $(document).ready(function () {
        var cbxmcratingreview_setting_awn_options = {
            labels: {
                tip: cbxmcratingreview_setting.awn_options.tip,
                info: cbxmcratingreview_setting.awn_options.info,
                success: cbxmcratingreview_setting.awn_options.success,
                warning: cbxmcratingreview_setting.awn_options.warning,
                alert: cbxmcratingreview_setting.awn_options.alert,
                async: cbxmcratingreview_setting.awn_options.async,
                confirm: cbxmcratingreview_setting.awn_options.confirm,
                confirmOk: cbxmcratingreview_setting.awn_options.confirmOk,
                confirmCancel: cbxmcratingreview_setting.awn_options.confirmCancel,
            },
        };
        ...
        $('.setting-color-picker-wrapper').each(function (index, element) {
            ...
            const pickr = Pickr.create({
                ...
                i18n: cbxmcratingreview_setting.pickr_i18n,
            });
        ...
        $('.selecttwo-select-wrapper').each(function (index, element) {
            ...
            $placeholder =
                $placeholder == ''
                    ? cbxmcratingreview_setting.please_select
                    : $placeholder;
            ...
            $element
                ...
                .on('select2:open', function () {
                    $('.select2-search__field').attr(
                        'placeholder',
                        cbxmcratingreview_setting?.search
                    );
                })
        ...
        var $setting_page = $('#cbxmcratingreview-setting');
        ...
        function setting_nav_change($tab_id) {
            ...
            if (typeof localStorage !== 'undefined') {
                localStorage.setItem('cbxmcratingreviewactivetab', clicked_group);
            }

            //load the reset items
            if (clicked_group === '#cbxmcratingreview_tools') {
                // $.ajax({
                //     type: 'post',
                //     dataType: 'json',
                //     url: cbxmcratingreview_setting.ajaxurl,
                //     data: {
                //         action: 'cbxmcratingreview_settings_reset_load',
                //         security: cbxmcratingreview_setting.nonce,
                //     },
                //     success: function (data, textStatus, XMLHttpRequest) {
                //         $('#cbxmcratingreview_resetinfo_wrap').html(data.html);
                //     }, //end of success
                // }); //end of ajax
            }
        ...
        $('#reset_data_trigger').on('click', function (e) {
            ...
            var notifier = new AWN(cbxmcratingreview_setting_awn_options);
            ...
            notifier.confirm(
                cbxmcratingreview_setting.are_you_sure_delete_desc,
                onOk,
                onCancel,
                {
                    labels: {
                        confirm: cbxmcratingreview_setting.are_you_sure_global,
                    },
                }
            );
        }); //end click #reset_data_trigger

        $('#cbxmcratingreview_resetinfo_wrap').on(
            'click',
            '.cbxmcratingreview_setting_options_check_action_call',
            function (e) {
                e.preventDefault();

                var $this = $(this);
                $('#cbxmcratingreview_resetinfo_wrap').find(':checkbox').prop('checked', true);
            }
        );

        $('#cbxmcratingreview_resetinfo_wrap').on(
            'click',
            '.cbxmcratingreview_setting_options_check_action_ucall',
            function (e) {
                e.preventDefault();

                var $this = $(this);
                $('#cbxmcratingreview_resetinfo_wrap').find(':checkbox').prop('checked', false);
            }
        );
        ...
        $('.form-table-fields-parent-wrap').on('click', '.form-table-fields-new', function (event) {
            ...
            if (!$busy) {
                ...
                $.ajax({
                    ...
                    url: cbxmcratingreview_setting.ajaxurl,
                    data: {
                        action: 'cbxmcratingreview_add_new_field',
                    ...
                    success: function (data, textStatus, XMLHttpRequest) {
                        ...
                        new AWN(cbxmcratingreview_setting_awn_options).success(data.message);
