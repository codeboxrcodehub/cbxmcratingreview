 ...
 */
namespace CBXMCRatingReviewScoped\Symfony\Component\CssSelector\Parser;

use CBXMCRatingReviewScoped\Symfony\Component\CssSelector\Exception\SyntaxErrorException;
use CBXMCRatingReviewScoped\Symfony\Component\CssSelector\Node;
use CBXMCRatingReviewScoped\Symfony\Component\CssSelector\Parser\Tokenizer\Tokenizer;
/**
 * CSS selector parser.
 *
...
class Parser implements ParserInterface
{
    ...
    private function parserSelectorNode(TokenStream $stream, bool $isArgument = \false): Node\SelectorNode
    {
        ...
        while (\true) {
            $stream->skipWhitespace();
            $peek = $stream->getPeek();
            if ($peek->isFileEnd() || $peek->isDelimiter([',']) || $isArgument && $peek->isDelimiter([')'])) {
                break;
            }
    ...
    private function parseSimpleSelector(TokenStream $stream, bool $insideNegation = \false, bool $isArgument = \false): array
    {
        ...
        while (\true) {
            $peek = $stream->getPeek();
            if ($peek->isWhitespace() || $peek->isFileEnd() || $peek->isDelimiter([',', '+', '>', '~']) || $isArgument && $peek->isDelimiter([')'])) {
                break;
            }
            if (null !== $pseudoElement) {
                throw SyntaxErrorException::pseudoElementFound($pseudoElement, 'not at the end of a selector');
