...
class LogFrontController {
	 ...
	 * @since 2.0.0
	 */
	public function getPublicLogs( WP_REST_Request $request ) {
		$response = new WP_REST_Response();
		$response->set_status( 200 );

		if ( ! is_user_logged_in() ) {
			$response->set_data( [
				'info'    => esc_html__( 'Unauthorized request', 'cbxmcratingreview' ),
				'success' => false
			] );
		...
		try {
			...
			$logs = RatingReviewLog::query()->with( 'user', 'post' )->where( 'user_id', $user_id );
			...
			if ( str_contains( $e->getMessage(), 'Base table or view not found' ) ) {
				...
				$response->set_data( [
					'info'    => esc_html__( 'Something Went Wrong. Please try again later1.', 'cbxmcratingreview' ),
					'success' => false
				] );
			}
		} catch ( Exception $e ) {
			$response->set_data( [
				'info'    => esc_html__( 'Something Went Wrong. Please try again later2.', 'cbxmcratingreview' ),
				'success' => false
			] );
		}

		return $response;
	} //end method getPublicLogs
	 ...
	 * @since 2.0.0
	 */
	public function getPublicLog( WP_REST_Request $request ) {
		$response = new WP_REST_Response();
		$response->set_status( 200 );
		...
		if ( isset( $request['id'] ) ) {
			$log = RatingReviewLog::where( 'id', absint( $request['id'] ) )->where( 'user_id', $user_id )->with( 'user', 'post', 'modUser', 'form' )->first();
	 ...
	 * @since 2.0.0
	 */
	public function savePublicLog( WP_REST_Request $request ) {
	...
	}//end method savePublicLog
}//end class LogFrontController