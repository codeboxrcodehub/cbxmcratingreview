...
/**
	...
	function callback_heading( $args ) {
		$plus_svg  = cbxmcratingreview_esc_svg( cbxmcratingreview_load_svg( 'icon_plus' ) );
		$minus_svg = cbxmcratingreview_esc_svg( cbxmcratingreview_load_svg( 'icon_minus' ) );
		...
		$html = '<h3 class="setting_heading"><span class="setting_heading_title">' . esc_html( $args['name'] ) . '</span><a title="' . esc_attr__( 'Click to show hide',
				'cbxmcratingreview' ) . '" class="setting_heading_toggle button outline primary icon icon-only icon-inline" href="#"><i class="cbx-icon setting_heading_toggle_plus">' . $plus_svg . '</i><i class="cbx-icon setting_heading_toggle_minus">' . $minus_svg . '</i></a></h3>';
...
/**
	...
	function callback_select( $args ) {
		...
		if ( $args['placeholder'] == '' ) {
			$placeholder         = $args['placeholder'] = esc_html__( 'Please Select', 'cbxmcratingreview' );
			$args['placeholder'] = esc_html__( 'Please Select', 'cbxmcratingreview' );
		} else {
	...
	function callback_page( $args ) {
		$edit_svg     = cbxmcratingreview_esc_svg( cbxmcratingreview_load_svg( 'icon_edit' ) );
		$external_svg = cbxmcratingreview_esc_svg( cbxmcratingreview_load_svg( 'icon_external' ) );
		...
		if ( $value > 0 ) {
			if ( $check_content != '' ) {
				...
				if ( has_shortcode( $page_content, $check_content ) ) {
					...
					$page_shortcode_note = sprintf( esc_html__( 'This page has shortcode %s', 'cbxmcratingreview' ), $check_content );
				} else {
					$shortcode_found_class = 'description_note_off';
					/* translators: %s: Shortcode name  */
					$page_shortcode_note = sprintf( esc_html__( 'This page doesn\'t have shortcode %s', 'cbxmcratingreview' ), $check_content );
				}
			}

			//edit
			if ( current_user_can( 'edit_post', $value ) ) {
				$page_html .= '<a class="setting_pages_action setting_pages_action_edit button primary icon icon-only icon-inline small" target="_blank" title="' . esc_attr__( 'Edit', 'cbxmcratingreview' ) . '" href="' . esc_url( get_edit_post_link( $value ) ) . '"><i class="cbx-icon">' . $edit_svg . '</i></a>';
			}

			//view
			$page_html .= '<a class="setting_pages_action setting_pages_action_view button outline primary icon icon-only icon-inline small" title="' . esc_attr__( 'View', 'cbxmcratingreview' ) . '" target="_blank" href="' . esc_url( get_the_permalink( $value ) ) . '"><i class="cbx-icon">' . $external_svg . '</i></a>';
		}
...
/**
	...
	function callback_color( $args, $value = null ) {
		...
		$choose_color = esc_html__( 'Choose Color', 'cbxmcratingreview' );
	...
	function callback_repeat( $args ) {
		$delete_svg = '<i class="cbx-icon">' . cbxmcratingreview_esc_svg( cbxmcratingreview_load_svg( 'icon_delete' ) ) . '</i>';
		$edit_svg   = '<i class="cbx-icon">' . cbxmcratingreview_esc_svg( cbxmcratingreview_load_svg( 'icon_edit' ) ) . '</i>';
		$sort_svg   = '<i class="cbx-icon">' . cbxmcratingreview_esc_svg( cbxmcratingreview_load_svg( 'icon_move' ) ) . '</i>';
		...
		if ( $allow_new ) {
			$html .= '<p style="text-align: center;"><a data-index="' . absint( $index ) . '" data-busy="0" data-field_name="' . $args['name'] . '" data-section_name="' . $section_name . '" data-option_name="' . $option_name . '" class="button secondary form-table-fields-new ld-ext-right" href="#">' . esc_html__( 'Add New',
					'cbxmcratingreview' ) . '<span class="ld ld-spin ld-ring"></span></a></p>';
		}
        ...
        <div id="setting-tabs-contents">
            <div id="global_setting_group_actions" class="mb-0">
                <a class="button outline primary global_setting_group_action global_setting_group_action_open pull-right"
                   href="#">
					<?php esc_html_e( 'Toggle All Sections', 'cbxmcratingreview' ); ?>
                </a>
            ...
            <div class="metabox-holder">
				...
				foreach ( $this->settings_sections as $form ):
                    ...
                    <div id="<?php echo esc_attr( $form['id'] ); ?>" class="global_setting_group"
                        ...
                        <form method="post" action="options.php" class="cbxmcratingreview_setting_form">
							<?php
							do_action( 'cbxmcratingreview_setting_form_top_' . $form['id'], $form );
							settings_fields( $form['id'] );
							do_settings_sections( $form['id'] );
							do_action( 'cbxmcratingreview_setting_form_bottom_' . $form['id'], $form );
							?>
                            <div class="global_setting_submit_buttons_wrap">
								<?php do_action( 'cbxmcratingreview_setting_submit_buttons_start', $form['id'] ); ?>
								<?php submit_button( esc_html__( 'Save Settings', 'cbxmcratingreview' ),
									'button primary submit_setting', 'submit', true,
									[ 'id' => 'submit_' . esc_attr( $form['id'] ) ] ); ?>
								<?php do_action( 'cbxmcratingreview_setting_submit_buttons_end', $form['id'] ); ?>
                            </div>
                        </form>
                    </div>
					<?php
	...
	function callback_file_extensions_checker( $args ) {
		...
		$image_ext_arr = CBXMCRatingReviewHelper::imageExtArr();
