...
if ( ! class_exists( 'CBXMCRatingReviewReviewStatusUpdateUserEmail', false ) ) :
	 ...
	 *
	 * Sending email alert to user when review status ic changed from admin
	 */
	class CBXMCRatingReviewReviewStatusUpdateUserEmail extends CBXMCRatingReviewEmail {
		/**
		 * Constructor.
		 */
		public function __construct() {
			$this->id          = 'review_status_change';
			$this->user_email  = true; //alert for admin
			$this->title       = esc_html__( 'Review status change notification', 'cbxmcratingreview' );
			$this->description = esc_html__( 'Sends notification to user on review creation.', 'cbxmcratingreview' );

			$this->template_html = 'emails/review_status_change.php';
			...
			add_action( 'cbxmcratingreview_review_status_change', [ $this, 'trigger' ], 10, 4 );
		 ...
		 */
		public function trigger( $old_status, $new_status, $new_review_info, $review_info_old ) {

			if ( $this->is_enabled() ) {

				$this->recipient = $new_review_info['user_email'];
		...
		public function get_default_subject() {
			return esc_html__( 'Review Status Change Notification', 'cbxmcratingreview' );
		...
		public function get_default_heading() {
			return esc_html__( 'Review Status Changed', 'cbxmcratingreview' );
	...
	}//end class CBXMCRatingReviewReviewStatusUpdateUserEmail
endif;

return new CBXMCRatingReviewReviewStatusUpdateUserEmail();