<?php
if ( ! function_exists( 'cbxmcratingreview_rgb_from_hex' ) ) {
	 ...
	 * @return array
	 */
	function cbxmcratingreview_rgb_from_hex( $color ) {
...
if ( ! function_exists( 'cbxmcratingreview_hex_darker' ) ) {
	 ...
	 * @return string
	 */
	function cbxmcratingreview_hex_darker( $color, $factor = 30 ) {
		$base  = cbxmcratingreview_rgb_from_hex( $color );
		$color = '#';

		foreach ( $base as $k => $v ) {
			$amount      = $v / 100;
			$amount      = cbxmcratingreview_round( $amount * $factor );
...
if ( ! function_exists( 'cbxmcratingreview_hex_lighter' ) ) {
	 ...
	 * @return string
	 */
	function cbxmcratingreview_hex_lighter( $color, $factor = 30 ) {
		$base  = cbxmcratingreview_rgb_from_hex( $color );
		$color = '#';
...
if ( ! function_exists( 'cbxmcratingreview_hex_is_light' ) ) {
	 ...
	 */
	function cbxmcratingreview_hex_is_light( $color ) {
...
if ( ! function_exists( 'cbxmcratingreview_light_or_dark' ) ) {
	 ...
	 * @return string
	 */
	function cbxmcratingreview_light_or_dark( $color, $dark = '#000000', $light = '#FFFFFF' ) {
		return cbxmcratingreview_hex_is_light( $color ) ? $dark : $light;
	}
}

if ( ! function_exists( 'cbxmcratingreview_format_hex' ) ) {
	 ...
	 * @return string|null
	 */
	function cbxmcratingreview_format_hex( $hex ) {
 ...
 */
function cbxmcratingreview_round( $val, int $precision = 0, int $mode = PHP_ROUND_HALF_UP ): float {
	if ( ! is_numeric( $val ) ) {
		$val = floatval( $val );
	}
 ...
 * @return float
 */
function cbxmcratingreview_array_sum( array $arr ): float {
